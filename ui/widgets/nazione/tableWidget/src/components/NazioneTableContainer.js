...
import { reducer, initialState } from 'state/nazione.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/nazione.types';
...
class NazioneTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { naziones, headers } = await apiNazionesGet(requestParameters);
        const nazioneCount = (headers && headers['X-Total-Count']) || null;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...naziones] : naziones,
            count: nazioneCount,
          },
        });
      } catch (err) {
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} itemCount={itemCount}>
            <div className={classes.tableWrapper}>
              <NazioneTable items={items} onSelect={onSelect} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
NazioneTableContainer.propTypes = {
...
NazioneTableContainer.defaultProps = {
  onAdd: () => {},
  onError: () => {},
  onSelect: () => {},
  paginationMode: '',
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPaginationContext(NazioneTableContainer))
  )
);
