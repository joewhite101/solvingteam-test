...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/nazione/formWidget/>):
    1. Run `npm i`
...
-   **nazione.form.createError** (fired when an error occurs during nazione creation)
-   **nazione.form.create** (fired when the nazione is successfully created)
-   **nazione.form.updateError** (fired when an error occurs during nazione update)
-   **nazione.form.update** (fired when the nazione is successfully updated)
