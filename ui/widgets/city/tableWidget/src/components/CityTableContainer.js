...
import { reducer, initialState } from 'state/city.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/city.types';
...
class CityTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { cities, headers } = await apiCitiesGet(requestParameters);
        const cityCount = (headers && headers['X-Total-Count']) || null;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...cities] : cities,
            count: cityCount,
          },
        });
      } catch (err) {
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} itemCount={itemCount}>
            <div className={classes.tableWrapper}>
              <CityTable items={items} onSelect={onSelect} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
CityTableContainer.defaultProps = {
  onAdd: () => {},
  onError: () => {},
  onSelect: () => {},
  paginationMode: '',
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPaginationContext(CityTableContainer))
  )
);
